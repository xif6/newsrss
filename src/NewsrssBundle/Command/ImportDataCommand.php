...
class ImportDataCommand extends ContainerAwareCommand
{
    ...
    protected function importFlux()
    {
        ...
        while ($news = $statement->fetch()) {
            ...
            if (!empty($news['site'])) {
                $site = $this->em->getRepository('NewsrssBundle:Site')->findOneByName($news['site']);
                $flux->setSite($site);
            }
    ...
    protected function importFluxHttp()
    {
        ...
        while ($news = $statement->fetch()) {
            ...
            $flux = $this->em->getRepository('NewsrssBundle:Flux')->findOneByUrl($news['url']);
    ...
    protected function importCategory()
    {
        ...
        foreach ($rubs as $rub) {
            if ($rub['parent_id']) {
                $category = $this->em->getRepository('NewsrssBundle:Category')->find($rub['id']);
                $parent = $this->em->getRepository('NewsrssBundle:Category')->find($rub['parent_id']);
    ...
    protected function importFluxCategory()
    {
        ...
        while ($newsrub = $statement->fetch()) {
            $flux = $this->em->getRepository('NewsrssBundle:Flux')->find($newsrub['flux_id']);
            $category = $this->em->getRepository('NewsrssBundle:Category')->find($newsrub['category_id']);
    ...
    public function importUserFlux()
    {
        ...
        while ($lognews = $statement->fetch()) {
            $user = $this->em->getRepository('NewsrssBundle:User')->find($lognews['user_id']);
            if (!empty($lognews['url'])) {
                $flux = $this->em->getRepository('NewsrssBundle:Flux')->findOneByUrl($lognews['url']);
            } else {
                $flux = $this->em->getRepository('NewsrssBundle:Flux')->find($lognews['flux_id']);
            }
